...
export class Map {
    ...
    constructor(game, mapId) {
        ...
        for(const place of mapInfo.places) {
            this.places.push(new Place(place.name, new Vector2(place.x, place.y)));
        }
        ...
        if(!Debug.minimalMapGeneration) {
            ...
            this.genObstacleTest("stone_01", new Vector2(450, 150));
        }

        this.groundPatches = [];
    }

    private genStructure(typeString: string, structureData: any, setPosition: Vector2 | null = null, setOrientation: number | null = null): void {
        let position;
        if(setPosition) position = setPosition;
        else if(Config.debugMode) position = new Vector2(450, 150);
    ...
    private genObstacleTest(typeString, position?: Vector2): void {
        this.genObstacle(typeString, position ?? new Vector2(455, 155), 0, 0, 1, Objects[typeString]);
    }
    ...
    private getRandomPositionFor(kind: ObjectKind, collisionData, scale?): Vector2 {
        if(!scale) scale = 1;
