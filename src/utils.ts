...
export interface IntersectResult { point: Vector2, normal: Vector2 }
export class Item {
    type: string;
    count: number;
...
export function randomVec(minX: number, maxX: number, minY: number, maxY: number): Vector2 { return new Vector2(random(minX, maxX), random(minY, maxY)); }
...
export function distanceBetween(v1: Vector2, v2: Vector2): number { return Math.sqrt(Math.pow(v1.x - v2.x, 2) + Math.pow(v1.y - v2.y, 2)); }

export function addOrientations(n1: number, n2: number): number {
    return (n1 + n2) % 4;
}

export function addAdjust(position1: Vector2, position2: Vector2, orientation: number): Vector2 {
    if(orientation === 0) return position1.add(position2);
    let xOffset, yOffset;
    switch(orientation) {
        case 1:
            xOffset = -position2.y;
    ...
    return position1.add(new Vector2(xOffset, yOffset));
}

export function rotateRect(pos: Vector2, min: Vector2, max: Vector2, scale: number, orientation: number): { min: Vector2, max: Vector2 } {
    min.x *= scale;
    min.y *= scale;
    max.x *= scale;
    max.y *= scale;
    if(orientation !== 0) {
        ...
        switch(orientation) {
            case 1:
                min = new Vector2(minX, maxY);
                max = new Vector2(maxX, minY);
                break;
            case 2:
                min = new Vector2(maxX, maxY);
                max = new Vector2(minX, minY);
                break;
            case 3:
                min = new Vector2(maxX, minY);
                max = new Vector2(minX, maxY);
                break;
        }
    }
    return {
...
export function bodyFromCollisionData(game: Game, data, pos: Vector2, orientation = 0, scale = 1): StaticBody | null {
    if(!data?.type) {
    ...
    if(data.type === CollisionType.Circle) {
        ...
        body = game.physics.add.staticBody(pos.x, pos.y);
        body.setCircle(data.rad * scale);
        ...
        body = game.physics.add.staticBody(x, y, width, height);
    }
    return body;
}

export function unitVecToRadians(v: Vector2): number {
...
export class SurvivBitStream extends BitStream {
    ...
    writeVec(vec: Vector2, minX: number, minY: number, maxX: number, maxY: number, bitCount: number): void {
        this.writeFloat(vec.x, minX, maxX, bitCount);
        this.writeFloat(vec.y, minY, maxY, bitCount);
    }

    readVec(minX: number, minY: number, maxX: number, maxY: number, bitCount: number): Vector2 {
        return new Vector2(this.readFloat(minX, maxX, bitCount), this.readFloat(minY, maxY, bitCount));
    }

    writeUnitVec(vec: Vector2, bitCount): void {
        this.writeVec(vec, -1, -1, 1, 1, bitCount);
    }

    readUnitVec(bitCount): Vector2 {
        return this.readVec(-1, -1, 1, 1, bitCount);
    }

    writeVec32(vec: Vector2): void {
